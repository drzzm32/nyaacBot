...
namespace dotNSASM
{
    public partial class NSASM
    {
        ...
        protected virtual void LoadFuncList()
        {
            ...
            funcList.Add("roll", (dst, src, ext) =>
            {
                if (src == null) return Result.ERR;
                if (dst == null) return Result.ERR;
                if (src.type != RegType.INT && src.type != RegType.FLOAT) return Result.ERR;
                if (dst.type != RegType.INT && dst.type != RegType.FLOAT) return Result.ERR;

                double a = src.type == RegType.INT ? (int)src.data : (float)src.data;
                double b = dst.type == RegType.INT ? (int)dst.data : (float)dst.data;

                double max = Math.Max(a, b);
                double min = Math.Min(a, b);
                double val = (max - min) * random.NextDouble() + min;

                if (ext != null)
                    funcList["out"](ext, null, null);

                Register reg = new Register();
                reg.readOnly = true;
                if (src.type == RegType.INT && dst.type == RegType.INT)
                {
                    reg.type = RegType.INT;
                    reg.data = (int)val;
                }
                ...
                {
                    reg.type = RegType.FLOAT;
                    reg.data = (float)val;
                }
                
                return funcList["prt"](reg, null, null);
            });
        }
    }
}
